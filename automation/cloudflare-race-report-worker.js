...
function includesFullName(text, names) {
  const normalized = text.toLowerCase();
  return names.some((name) => normalized.includes(name.toLowerCase()));
}

function incompleteMemberNameReason(body) {
  const matches = [];
...
function clarificationReason(command, body, requestedAttachments) {
...
function discordModalInput(interaction, customId) {
...
function discordTextAttachmentFromOption(interaction) {
...
async function discordTextAttachment(interaction) {
  ...
  const bytes = new Uint8Array(await response.arrayBuffer());
  if (!bytes.length || bytes.length > MAX_DISCORD_TEXT_ATTACHMENT_BYTES) {
...
async function processDiscordRecapFileSubmission(env, interaction, inlineBody = "") {
...
function discordOpenRecapModal(defaultPolish, body = "", links = "", reason = "") {
  return new Response(
    JSON.stringify({
...
function discordOpenEventModal(body = "", links = "", reason = "") {
  return new Response(
    JSON.stringify({
      type: DISCORD_RESPONSE_MODAL,
      data: {
...
async function handleDiscordInteraction(request, env, ctx, rawBody) {
  ...
  if (!body) {
    ...
    if (hasRecapFile) {
      ctx.waitUntil(
        processDiscordRecapFileSubmission(env, interaction).catch((error) => (
  ...
  if (hasRecapFile) {
    ctx.waitUntil(
      processDiscordRecapFileSubmission(env, interaction, body).catch((error) => (
        discordFollowup(interaction, `I received the recap file, but the background update failed: ${error.message}\n\n${DISCORD_FALLBACK_MESSAGE}`)
          .catch((followupError) => console.error(followupError))
      )),
    );
